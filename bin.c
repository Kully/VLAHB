...
int bin(const int lo, const int hi)
{
...
int main(int argc, char* argv[])
{
    if(argc != 3)
    {
        fprintf(stderr, "./a.out in.hex out.bin\n");
        exit(1);
    }
    FILE* const fi = fopen(argv[1], "r");
    FILE* const fo = fopen(argv[2], "w");
    if(fi == NULL) fprintf(stderr, "could not open input  : %s\n", argv[1]);
    if(fo == NULL) fprintf(stderr, "could not open output : %s\n", argv[2]);
